...
function betterCourses() {
  ...
  courses.forEach((course, i) => {
    if(course.inPerson) courses[i].inPerson = false;
    if(course.homework) courses[i].homework = false;
  });
...
function findPairs() {
  ...
  lettersToPair.forEach((ltr, i) => {
    for(let k=i+1; k<lettersToPair.length; k++){
      if(ltr === lettersToPair[k]){
        pairsArray.push([i, k]);
      }
    }
  });
...
function getEllensInfo() {
  return {...contactInfo, ...shippingInfo, name: "Ellen", email: "ellen@email.com"};
}
